...
def is_main_chapter(text: str):
    """只抓主章節標題，支援一到三十九，排除含「之」的子標題"""
    m = re.match(r'^(三十[一二三四五六七八九]|二十[一二三四五六七八九]|十[一二三四五六七八九]|[一二三四五六七八九十]+)、【([^】]+)】', text.strip())
    if m and '之' not in m.group(1):
...
def db_clear_all_chapters():
    """清除所有舊章節資料（重新上傳時呼叫）"""
    try:
        supabase.table("chapter_images").delete().neq("id", 0).execute()
        supabase.table("chapter_content").delete().neq("id", 0).execute()
